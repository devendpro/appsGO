package main

import (
	"fmt"
	"os"
	"strconv"
)

func unidadeValida(un string) (bool, string) {
	/* 	if un == "celsius" {
	   		return true, "fahrenheit"
	   	} else if un == "quilometros" {
	   		return true, "milhas"
	   	} else {
	   		return false, "informe uma unidade válida"
	   	} */

	switch un {
	case "celsius":
		{
			return true, "fahrenheit"
		}
	case "quilometros":
		{
			return true, "milhas"
		}
	default:
		{
			return false, "Unidade de conversão inválida"
		}
	}

}

func main() {
	if len(os.Args) < 3 {
		fmt.Println("Uso: conversor <valores> <unidade>")
		os.Exit(1)
	} else {
		unidadeOrigem := os.Args[len(os.Args)-1]
		valoresOrigem := os.Args[1 : len(os.Args)-1]
		if ok, unidadeDestino := unidadeValida(unidadeOrigem); ok {
			for i, v := range valoresOrigem {
				valorOrigem, err := strconv.ParseFloat(v, 64)
				if err != nil {
					fmt.Printf("O valor %s na posição %d não é um número válido\n", v, i)
					os.Exit(i)
				} else {
					valorDestino := 0.0
					if unidadeOrigem == "celsius" {
						valorDestino = valorOrigem*1.8 + 32
					} else {
						valorDestino = valorOrigem / 1.30934
					}

					fmt.Printf("%.2f %s = %.2f %s\n", valorOrigem, unidadeOrigem, valorDestino, unidadeDestino)
				}
			}

		} else {
			fmt.Println(unidadeDestino)
			os.Exit(1)
		}

	}

}
